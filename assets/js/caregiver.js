...
if (kcCards.length && kcTrack) {
   ...
   const startDragging = (e) => {
      isDragging = true
      kcTrack.style.transition = 'none'
      startX = (e.pageX || e.touches[0].pageX) - kcTrack.offsetLeft
   ...
   const moveDragging = (e) => {
      ...
      e.preventDefault()
      const x = (e.pageX || e.touches[0].pageX) - kcTrack.offsetLeft
      const walk = (x - startX)
      currentOffset = scrollLeft - walk
      
      const maxOffset = kcTrack.scrollWidth - kcTrack.parentElement.offsetWidth
      currentOffset = Math.max(-50, Math.min(currentOffset, maxOffset + 50)) // slight elastic
   ...
   const stopDragging = () => {
      if (!isDragging) return
      isDragging = false
      const cardW = kcCards[0].offsetWidth + 20
      const containerW = kcTrack.parentElement.offsetWidth
      
      // Calculate which card is closest to center
      // (currentOffset + containerW/2 - 120) / cardW
      const estimatedIndex = Math.round((currentOffset + containerW / 2 - 120 - (kcCards[0].offsetWidth / 2)) / cardW)
      kcGoTo(estimatedIndex)
   }

   // Event Listeners
   kcTrack.parentElement.addEventListener('mousedown', startDragging)
   window.addEventListener('mousemove', moveDragging)
   window.addEventListener('mouseup', stopDragging)

   kcTrack.parentElement.addEventListener('touchstart', startDragging, { passive: false })
   ...
   let kcAuto = setInterval(() => { if(!isDragging) kcGoTo(kcCurrent + 1) }, 4000)
   kcTrack.parentElement.addEventListener('mouseenter', () => clearInterval(kcAuto))
   kcTrack.parentElement.addEventListener('mouseleave', () => {
      kcAuto = setInterval(() => { if(!isDragging) kcGoTo(kcCurrent + 1) }, 4000)
   })
}
